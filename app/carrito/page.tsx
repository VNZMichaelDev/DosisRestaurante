...
export default function CarritoPage() {
  ...
  const placeOrder = async () => {
    ...
    for (const item of items) {
      const row = stockRows?.find((r: { id: string }) => r.id === item.id);
      if (row && row.stock === 0) {
        outOfStock.push(row.name);
      } else if (row && typeof row.stock === "number" && row.stock < item.qty) {
        tooMuch.push(`${row.name} (solo quedan ${row.stock})`);
      }
    }

    if (outOfStock.length > 0) {
      setError(`Agotado: ${outOfStock.join(", ")}. Retira estos productos del carrito.`);
      setLoading(false);
      return;
    }
    if (tooMuch.length > 0) {
      setError(`Stock insuficiente: ${tooMuch.join(", ")}. Ajusta las cantidades.`);
      setLoading(false);
      return;
    }
    ...
    // Decrementar stock de cada producto (fire-and-forget, atómico)
    for (const item of items) {
